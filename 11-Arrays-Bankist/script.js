...
btnTransfer.addEventListener('click', function (e) {
  ...
  // Add negative movement to user (remember to check if values are available)
  if (
    amount > 0 &&
    receiverAcc &&
...
// SLICE - Recorta certa parte mantendo o array igual - NAO MUDA o array original
...
// SPLICE - Recorta certa parte retirando do array (DELETA) - MUDA o array original
...
// Ja sabemos push, unshift, pop, shift, indexOf, includes
...
console.log('Anderson'.at(0));
console.log('Anderson'.at(4));
*/
...
for (const [i, movement] of movements.entries()) {
  if (movement > 0) {
    console.log(`Movimentacao ${i + 1}: Voce depositou ${movement}`);
  } else {
    console.log(`Movimentacao ${i + 1}: Voce sacou ${Math.abs(movement)}`);
  }
}

console.log('-----forEach-----');

// parametros: elemento, indice e array inteiro
movements.forEach(function (mov, i, arr) {
  if (mov > 0) {
    console.log(`Movimentacao ${i + 1}: Voce depositou ${mov}`);
  } else {
    console.log(`Movimentacao ${i + 1}: Voce sacou ${Math.abs(mov)}`);
  }
});

// continue and break dont work in forEach, so if you need to use it, you
// will need to use for of
...
// parametros: valor, chave, mapa inteiro
// MAP
...
A dog is an adult if it is at least 3 years old, and it's a puppy if it's
...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  allDogs.forEach(function (dogAge, i) {
    const adultOrPuppy =
    dogAge > 2 ? `an adult, and is ${dogAge} years old` : `still a puppy 🐶`;
    
    const log = `Dog number ${i + 1} is ${adultOrPuppy} `;
...
MAP  
similar ao forEach
Map cria e retonar um novo array baseado no original

FILTER
filtro o array de entrada de acordo com uma condicao
Retorna um novo array

REDUCE
reduz o array de entrada a um unico elemento de acordo com o comando
...
const movementsDescriptions = movements.map(
  (mov, i) =>
    `Movimentacao ${i + 1}: Voce ${mov > 0 ? 'depositou' : 'sacou'} ${Math.abs(mov)}`
);
...
// reduce(function, acc star value)
...
// REPARE NA ELEGANCIA DA PRIMEIRA
...
///////////////////////////////////////
// FIND
// metodo que retorna o primeiro elemento de um array que satisfaz uma condicao
const movements = [200, 450, -400, 3000, -650, -130, 70, 1300];
...
// EVERY: retorna true se todos os elementos respeitam a condicao
...
// .2
const sarahsDog = dogs.forEach(function (dog) {
...
const eatsExact = dogs.some(dogs => dogs.curFood === dogs.recFood);
